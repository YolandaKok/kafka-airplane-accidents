...
public class ProducerService {
  ...
  public void produceAccidentRawRecords() {
    ...
    var res =
        accidents.stream()
            .filter(x -> x.getSpeciesName() != null)
            .filter(
                x -> x.getSpeciesName().equals("UNKNOWN SMALL BIRD") && x.getIncidentYear() == 2015)
            .count();
    ...
    accidents.forEach(
        accident -> {
          try {
            kafkaProducer.sendMessageWithKeySync(rawTopic, accident.getRecordId(), accident);
