...
public class AccidentConsumer {
  ...
  @KafkaListener(
      topics = "${app.kafka.topics.accidents.topN}",
      groupId = "con-5",
      concurrency = "5",
  ...
  public void listen(ConsumerRecord<String, String> consumerRecord) throws JsonProcessingException {
    List<TopAccident> topAccidents =
        objectMapper.readValue(consumerRecord.value(), new TypeReference<>() {});

    List<AccidentResult> results =
        topAccidents.stream()
            .map(
                item ->
                    AccidentResult.builder()
                        .count(item.getCount())
    ...
    accidentResultRepository.saveAll(results);
  }
}
